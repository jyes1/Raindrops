...
void setup() {
  ...
  index = 1; //sets the index to 1. It must be 1 so that in a later for statement in the void draw section, i = 0.
  ...
  x = 100; //declares the x coordinate of the start / game over rectangle
  y = 210; //declares the y coordinate of the start / game over rectangle
  w = 300; //declares the width of the start / game over rectangle
  h = 100; //declares the height of the start / game over rectangle
...
void draw() {
  ...
  if (!gameOver) { //if game over is now true...
    if (run) { //if the program runs... (most other things are included here)
      ...
      image(cloud, random(297, 303), random(17, 23), WidthC, HeightC); //this creates the image of the cloud. it vibrates / moves. the lightning goes before this picture so it slashes behind the cloud.
  ...
  if (!run) { //if it's not running (AKA if it hasn't been started yet...)
    fill(0, 0, 255); //there is a blue...
    rect(100, 192.5, 300, 100); //box of these parameters.
    fill(240, 195, 14); //and also with a gold fill...
  ...
  if (missed > 49) { //if the player misses more than 50 raindrops
    ...
    fill(0, 0, 255); //and a blue...
    rect(x, y, w, h); //rectangle with previously defined parameters.
    fill(240, 195, 14); //in addition there will be in gold text...
  ...
  if (gameOver) { //if game over is true...
    if (mousePressed) { //and the mouse is pressed...
      if (mouseX > x && mouseX < x+w && mouseY > y && mouseY < y+h) { //within the parameters of the box...
        exit(); //the window closes
      }
    }
  }
}
